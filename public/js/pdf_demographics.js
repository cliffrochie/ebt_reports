...
function generateDemographicsPDF() {
    ...
    var chart1 = document.getElementById('operating_unit_population');
    var dataURL1 = chart1.toDataURL("image/png");

    var chart2 = document.getElementById('business_unit_population');
    var dataURL2 = chart2.toDataURL('image/png');

    var chart3 = document.getElementById('category_population')
    var dataURL3 = chart3.toDataURL();

    var chart4 = document.getElementById('gender_population')
    ...
    doc.text('No. of Employees per Operating Unit', 100, 140);
    doc.addImage(dataURL1, 'PNG', 20, 150, width, height);

    doc.text('No. Operating Unit per Business Unit', 380, 140);
    doc.addImage(dataURL2, 'PNG', 300, 150, width, height);

    doc.text('No. of Employees per Category', 100, 340);
    doc.addImage(dataURL3, 'PNG', 20, 350, width, height);
    
    doc.text('No. of Employees per Gender', 380, 340);
